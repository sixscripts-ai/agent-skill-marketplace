...
import { Bell, BookOpen, Boxes, Hammer, LogIn, Moon, Search, Settings2, Sparkles, Sun, TerminalSquare } from "lucide-react";
...
export function AppShellClient({
  ...
  initialUser?: MarketplaceUser | null;
}) {
  const pathname = usePathname();
  const router = useRouter();
  const [search, setSearch] = useState("");
  const [notificationsOpen, setNotificationsOpen] = useState(false);
  const [user, setUser] = useState<MarketplaceUser | null>(initialUser);
  ...
  function toggleTheme() {
    ...
    try {
      localStorage.setItem("agent-skills-cyan-theme", next);
    } catch {
      /* ignore */
    }
    document.querySelector('meta[name="theme-color"]')?.setAttribute("content", next === "light" ? "#ffffff" : "#0a0a0a");
  }
