...
export function ApiSettingsModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    try { const stored = localStorage.getItem("ai_api_keys"); if (stored) setKeys(JSON.parse(stored) as ApiKeys); }
    catch { setKeys({ openai: "", anthropic: "", google: "", xai: "", groq: "" }); }
  }, [isOpen]);
  function save() { localStorage.setItem("ai_api_keys", JSON.stringify(keys)); onClose(); }
  if (!isOpen) return null;
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="api-settings-title">
    <div className="relative w-full max-w-lg rounded-xl border border-border bg-card p-6 shadow-2xl">
      ...
      <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-lg bg-primary/10 text-primary"><KeyRound className="size-5"/></span><div><h2 id="api-settings-title" className="text-lg font-semibold text-foreground">API keys</h2><p className="mt-1 text-sm text-muted-foreground">Stored only in this browser.</p></div></div>
      ...
      <div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onClose} className="builder-secondary-button">Cancel</button><button type="button" onClick={save} className="builder-primary-button">Save keys</button></div>
    </div>
  </div>;
}
...
function KeyField({ label, placeholder, value, onChange }: { label: string; placeholder: string; value: string; onChange: (value: string) => void }) {
  return <label className="block"><span className="text-sm font-medium text-foreground">{label} API key</span><input type="password" placeholder={placeholder} value={value} onChange={(event)=>onChange(event.target.value)} className="builder-input mt-2 font-mono"/></label>;
}
