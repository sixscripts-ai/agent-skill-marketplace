...
type BuildEvent = { id: string; sequence: number; type: string; status: string; title: string; detail?: string | null; metadata?: Record<string, unknown> | null; createdAt: string };
type BuildRun = { id: string; status: string; model: string; prompt: string; currentBatch: number; error?: string | null; createdAt: string; events: BuildEvent[] };
type StoredProject = {
  id: string;
  project: AgentProject;
  conversation?: { messages?: Array<{ id: string; role: string; content: string; metadata?: { plan?: string[]; files?: string[] } }> };
  runs?: BuildRun[];
};
...
export function EveAiChat() {
  ...
  async function refreshRuns(id: string) {
    try {
      ...
      setRuns(stored.runs ?? []);
    } catch { /* ignore refresh failures during builds */ }
  }
    ...
    try {
      ...
      do {
        ...
        if (result.status === "clarify") {
          ...
          await api("/api/eve/runs", { method: "PATCH", body: JSON.stringify({ runId, status: "completed", currentBatch: batches, event: { type: "clarification", status: "waiting", title: "Clarification requested", detail: text, metadata: { requestId: response.requestId, plan: result.plan } } }) });
      ...
      await api("/api/eve/runs", { method: "PATCH", body: JSON.stringify({ runId, status: "completed", currentBatch: batches, event: { type: "complete", status: "completed", title: "Project saved", detail: text, metadata } }) });
      setProject(finalProject); setMessages([...history, { id: crypto.randomUUID(), role: "assistant", content: text, plan: metadata.plan, files: metadata.files }]);
      await refreshRuns(id);
    } catch (error) {
      const text = error instanceof Error ? error.message : String(error);
      const requestId = text.match(/Reference:\s*([0-9a-f-]{36})/i)?.[1];
      setMessages((current) => [...current, { id: crypto.randomUUID(), role: "assistant", content: text, error: true }]);
      if (id) {
        await api("/api/eve/messages", { method: "POST", body: JSON.stringify({ projectId: id, role: "assistant", content: text, metadata: { error: true, requestId } }) }).catch(() => undefined);
        if (runId) {
          await api("/api/eve/runs", { method: "PATCH", body: JSON.stringify({ runId, status: "failed", error: text, event: { type: "error", status: "failed", title: "Client build failed", detail: text, metadata: { requestId } } }) }).catch(() => undefined);
          await refreshRuns(id);
        }
      }
          ...
          <div className="eve-build-run">
            ...
            <ol className="eve-build-events">
              {latestRun.events.map((event) => {
                ...
                return <li key={event.id} className={`eve-build-event ${event.status}`}>
