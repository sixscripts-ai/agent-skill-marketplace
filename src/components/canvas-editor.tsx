...
const initialNodes: Node[] = [
  {
    ...
    position: { x: 250, y: 25 },
    className: "border-neutral-900 rounded-none bg-[color-mix(in_srgb,var(--fb-heat,var(--brand,#06b6d4))_12%,#fff)] font-mono text-xs",
  },
];
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      const newNode: Node = {
        ...
        className: "border border-neutral-900 rounded-none bg-[color-mix(in_srgb,var(--fb-heat,var(--brand,#06b6d4))_12%,#fff)] font-mono text-xs p-3 min-w-[150px] shadow-[2px_2px_0_0_rgba(0,0,0,1)]",
      };
    ...
    <div className="flex h-full w-full flex-row">
      ...
      <aside className="w-64 border-r border-neutral-300 bg-neutral-50 p-4">
        ...
        <div className="flex flex-col gap-3">
          <div
            className="cursor-grab border border-neutral-900 bg-[color-mix(in_srgb,var(--fb-heat,var(--brand,#06b6d4))_12%,#fff)] p-3 font-mono text-xs shadow-[2px_2px_0_0_rgba(0,0,0,1)] transition-transform hover:-translate-y-0.5 hover:shadow-[3px_3px_0_0_rgba(0,0,0,1)]"
            onDragStart={(event) => {
      ...
      {selectedNode && selectedNode.type && nodeTypeLabels[selectedNode.type] ? (
        <>
          ...
          <div className="fixed top-0 right-0 bottom-0 w-[380px] bg-[color-mix(in_srgb,var(--fb-heat,var(--brand,#06b6d4))_12%,#fff)] border-l border-neutral-200 z-50 shadow-[-4px_0_24px_rgba(0,0,0,0.08)] flex flex-col animate-in slide-in-from-right">
            <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-200">
              <h3 className="text-sm font-semibold text-neutral-950">{nodeTypeLabels[selectedNode.type].label}</h3>
              <button onClick={closePanel} className="grid size-7 place-items-center rounded-md border border-neutral-200 bg-[color-mix(in_srgb,var(--fb-heat,var(--brand,#06b6d4))_12%,#fff)] text-sm text-neutral-500 hover:bg-neutral-100" type="button">✕</button>
            </div>
