...
export function BuilderInspector({
  ...
  return <div className="space-y-4">
    <BuilderPanel title="Validation" description="Resolve every blocking issue before publishing.">
      ...
      {publishedSlug ? <div className="mt-4 rounded-lg border border-primary/30 bg-primary/10 p-4 text-sm text-foreground"><div className="flex items-center gap-2 font-semibold"><PackageCheck className="size-4 text-primary" />Version published</div><div className="mt-2 font-mono text-xs">/skills/{publishedSlug}</div>{savedUrls ? <div className="mt-3 flex flex-wrap gap-2"><ResultLink href={savedUrls.run}>Quick run</ResultLink><ResultLink href={savedUrls.detail}>Detail</ResultLink><ResultLink href={savedUrls.mySkills}>My Skills</ResultLink><ResultLink href={savedUrls.run}>Run</ResultLink><ResultLink href={savedUrls.edit}>Edit</ResultLink>{visibility === "public" ? <ResultLink href={savedUrls.marketplace}>Marketplace</ResultLink> : null}</div> : null}</div> : null}
